...
public class WorldGenHevea implements IWorldGenerator {
	...
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		...
		if (world.getBiomeGenForCoords(chunkX, chunkZ) instanceof TFCBiome) // Fixes ClassCastException
		{
			...
			tl0 = TFC_Climate.getTreeLayer(world, chunkX, Global.worldHeightAverage, chunkZ, 0);
			tl1 = TFC_Climate.getTreeLayer(world, chunkX, Global.worldHeightAverage, chunkZ, 1);
			tl2 = TFC_Climate.getTreeLayer(world, chunkX, Global.worldHeightAverage, chunkZ, 2);
	...
	private void generateTree(Random random, int chunkX, int chunkZ, World world) {
		...
		int xCoord = chunkX;
		int yCoord = Global.worldHeightAverage + 1;
		int zCoord = chunkZ;
